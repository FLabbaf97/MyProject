...
def prepare_dataloader(opt, data_type):  # TODO: complete this for other data also
    """ Load data and prepare dataloader. """

    if (data_type == 'gene_expression'):
        """ Load gene features and prepare dataloader for that"""
        cell_features = pd.read_csv(opt['cell_AE_config']['data'])
    ...
    # Split the data into a training set and a test set
        ...
        cell_fearute_train_dataloader = DataLoader(
            cell_feature_train_dataset, collate_fn=collate_fn, batch_size=opt['cell_AE_config']['batch_size'], shuffle=True)
        cell_feature_test_dataset = CellGeneDataset(cell_features_test)
        cell_fearute_test_dataloader = DataLoader(
            cell_feature_test_dataset,            collate_fn=collate_fn,
            batch_size=opt['cell_AE_config']['batch_size'], shuffle=True)
        return cell_fearute_train_dataloader, cell_fearute_test_dataloader

    elif data_type == 'comb_data':
        return
    elif data_type == 'drug':
        return
    else:
        print('error in preparing dataloader: incorrect data type')
...
def config(opt, justLoad=False):
    ...
    #print stuff
    # print("num_celllines", len(codes['cell'].idx2item))
    # print("num_drugs", len(codes['drugs'].idx2item))
    # print("num_tissue", len(codes['tissue'].idx2item))
    # print("num_disease", len(codes['disease'].idx2item))
    # print("num_drug_fp", len(drug_features.loc[0, 'fps']))
    # print("max_drug_sm_len", drug_features['smiles'].apply(lambda x: len(x)).max())
